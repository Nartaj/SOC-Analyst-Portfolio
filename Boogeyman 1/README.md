...
5. Forensic Recovery & Impact Assessment

   Для финальных разделов 5 и 6 тебе нужно показать, что расследование не просто выявило атаку, но и привело к конкретным результатам (что именно украли) и мерам защиты.

Ниже — профессиональный текст для твоего README.md.

5. Forensic Recovery & Impact Assessment

  Целью данного этапа было определение объема украденных данных и их содержимого. 
  После успешной реконструкции файла protected_data.kdbx из сетевого трафика,
  был   проведен анализ его содержимого.

    Метод доступа: Для открытия базы данных KeePass был использован мастер-
    пароль %p9^3!lL^Mz47E2GaT^y, ранее извлеченный из базы данных Sticky Notes 
    (plum.sqlite).

    Результаты анализа: Внутри зашифрованного хранилища были обнаружены:
      
      Учетные данные от внутренних корпоративных ресурсов.

      Номер и CVV-код кредитной карты жертвы.

    Бизнес-риски: Подтверждена полная компрометация финансовых данных
    сотрудника и потенциальный риск несанкционированного доступа к другим узлам
    сети через украденные пароли.

6. Recommendations & Mitigation

   На основе анализа TTPs (Tactics, Techniques, and Procedures) группировки Boogeyman,
   предлагаются следующие меры по укреплению безопасности:

     Технические меры:

       1) Сетевая фильтрация: Настроить мониторинг и блокировку аномального DNS-
           трафика. Внедрить системы инспекции DNS (DNS Firewall) для обнаружения туннелирования.

       2) Hardening PowerShell: Включить Constrained Language Mode для обычных
           пользователей и настроить централизованный сбор логов (Event ID 4104) в SIEM-
           систему для оперативного реагирования.
        
       3) Защита вложений: Запретить выполнение .lnk файлов из почтовых вложений и
           архивов. Настроить автоматическую проверку паролей к архивам в почтовом шлюзе
           (если поддерживается).

       4) Контроль ПО: Ограничить использование системных утилит типа nslookup и
           sqlite3/sq3 для обычных пользователей (принцип минимальных привилегий).
